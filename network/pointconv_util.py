...
def index_points_group(points, knn_idx):
    ...
    # 多卡跑的时候数据不在同一个卡上报错进行的修改
    # return new_points.cuda(device=points.device)
...
def sample_and_group(sample_idx, npoint, radius, nsample, xyz, xyz_raw, label,
    ...
    new_label = index_points_gather(label, sample_idx)  # [B, N, 3]
    ...
    if xyz_raw is not None:
        ...
        return new_xyz, new_label, new_points, sample_idx  # (b, n, 3) (b, n, 3) (b, n, nample, 3+c) (b, n)
...
class PointNetSaModule(nn.Module):
    ...
    def __init__(self, npoint: int, radius: float, nsample: int, in_channels: int, mlp: list, mlp2: list,
        ...
        :param radius: KNN的半径
        :param nsample: KNN的K
        :param in_channels: =  points的维度
        :param mlp: mlp[-1]决定了采样后得到的点的特征维度
        :param mlp2: mlp2[-1]决定了采样后得到的点的特征维度
        :param group_all:
        :param is_training:
        :param bn_decay:
        :param bn:
    ...
    def forward(self, xyz, xyz_raw, label, points, sample_idx=None):
        ...
        :param xyz: [B, N, 3],input points position data
        :param xyz_raw:[B, N,3]
        :param label: [B, N,3]
        :param points: [B, N, C], point cloud characteristics
        :param sample_idx:
        :return:
            new_xyz: [B, N, 3],sampled points position data
            new_label: [B, N, 3]
            new_points: [B, N, mlp2[-1]] if mlp2 is not None else [B, N, mlp1[-1]]
            sample_idx: 采样点的位置
        """

        # 这里xyz_raw只会影响最终是否有个new_xyz_raw，丝毫不影响其他的返回值
        if xyz_raw is not None:
            # [B,N,3] [B,N,3] [B,N,nsample,3+C] [B,N,3]
            # new_xyz,new_label和 new_xyz_raw 都是通过相同的索引分别对xyz,label和xyz_raw采样得到的
            new_xyz, new_label, new_points, new_xyz_raw, sample_idx = sample_and_group(sample_idx, self.npoint,
                                                                                       self.radius,
                                                                                       self.nsample, xyz, xyz_raw,
                                                                                       label, points, self.knn,
                                                                                       self.use_xyz)
        else:
            new_xyz, new_label, new_points, sample_idx = sample_and_group(sample_idx, self.npoint, self.radius,
                                                                          self.nsample, xyz, xyz_raw,
        ...
        if self.pooling == 'max':
            new_points = torch.max(new_points, dim=2, keepdim=True)[0]  # (b, npoint, 1, mlp[-1])
        elif self.pooling == 'avg':
            new_points = torch.mean(new_points, dim=2, keepdim=True)  # (b, npoint, 1, mlp[-1])

        if self.mlp2 is not None:
            for i, conv in enumerate(self.mlp2_convs):
                new_points = conv(new_points)

        new_points = new_points.squeeze(2)  # (b,npoint, mlp2[-1]) if mlp2 is not None else  (b,npoint, mlp[-1])
...
class CostVolume(nn.Module):
    def __init__(self, radius: float, nsample: int, nsample_q: int, in_channels: int, mlp1: list, mlp2: list,
                 ...
                 is_fusion: bool = False, cost_local: bool = False, ):
        """

        :param radius:
        :param nsample: KNN的K
        :param nsample_q: KNN的N
        :param in_channels: = forward() 点的特征的维度
        :param mlp1: 用于生成pi_feat1_new的MLP
        :param mlp2: 用于生成pi_feat1_new对应权重的MLP,mlp1[-1]=mlp2[-1]
        :param is_training:
        :param bn_decay:
        :param bn:
        :param pooling:
        ...
        :param corr_func:
        :param is_fusion:
        :param cost_local: 是否计算周围点的cost volume
        """
    ...
    def forward(self, warped_xyz, warped_points, f2_xyz, f2_points):
        ...
        costVolume_self = torch.sum(pc_feat1_new, dim=2, keepdim=False)  # [B,N, mlp2[-1]]

        if self.fusion:  # 这个参数是为了当时进行点云融合而设置的
            return f2_xyz, 0.5 * costVolume_self + 0.5 * f2_points
        elif self.local:
            ...
            return costVolume_self, costVolume_local
        else:
            return costVolume_self
...
class Occ_CostVolume(nn.Module):
    ...
    def forward(self, warped_xyz, warped_points, f2_xyz, f2_points, up_occ_mask=None):
        ...
        :param f2_xyz: [B,N,3]
        :param f2_points: [B,N,C]
        :param up_occ_mask: [B,1,N],forward中有up_occ_mask时，is_bottom一定是false
        :return: [B,N,C,mlp2[-1]]
        """
...
class Occ_weighted_CV(nn.Module):
    ...
    def forward(self, warped_xyz, warped_points, f2_xyz, f2_points, up_occ_mask=None):
        ...
        # costVolume_cross = self.cost_cross_cv(warped_xyz, warped_points, f2_xyz, f2_points)  # [B, N, mlp2[-1]]
        # knn_idx_self = knn_point(self.nsample + 32, warped_xyz, warped_xyz)  # (B, N, K)
        # costVolume_self = index_points_group(costVolume_cross, knn_idx_self)  # (B, N, K, mlp2[-1])
        # costVolume_self = torch.max(costVolume_self, 2)[0]  # (B, N, mlp2[-1])
        # costVolume = occ_mask * costVolume_cross + (1.0 - occ_mask) * costVolume_self  # (B, N, mlp2[-1])
...
class SetUpconvModule(nn.Module):
    def __init__(self, nsample: int, in_channels: list, mlp: list, mlp2: list, is_training: bool,
        ...
        :param nsample:
        :param in_channels: in_channels[0]=feat1的特征维度C1，in_channels[-1]=feat2的特征维度C2
        :param mlp: C2+3 = in_channels[-1]+3 --> mlp[0] --> mlp[-1]
        :param mlp2: mlp[-1]+C1 =  mlp[-1]+in_channels[0] --> mlp2[0]-->mlp2[-1]
        :param is_training:
        :param bn_decay:
        :param bn:
        :param pooling:
...
class FlowPredictor(nn.Module):
    ...
    def __init__(self, in_channels, mlp, is_training, bn_decay, bn=True):
        ...
        :param in_channels: = forward()参数的维度之和
