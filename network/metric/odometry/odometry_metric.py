...
class kittiOdomEval():
    def __init__(self, config):
        ...
        self.pre_result_dir = config.pre_result_dir
        self.epoch = config.epoch
        # self.eval_seqs = config.eva_seqs
        self.eval_seqs = []
        self.visual_save_root = config.visual_save_root

        # evalute all files in the folder
        if config.eva_seqs == '*':
            if not os.path.exists(self.pre_result_dir):
                print('File path error!')
                exit()
            if os.path.exists(self.pre_result_dir + '/all_stats.txt'):
                os.remove(self.pre_result_dir + '/all_stats.txt')
            files = glob.glob(self.pre_result_dir + '/*.txt')
            assert files, "There is not trajectory files in: {}".format(self.pre_result_dir)
            for f in files:
    ...
    def plot_rpy(self, seq, poses_ref, poses_pred, plot_path_dir, axes='szxy'):
        ...
        pred_rpy = np.array([euler_from_matrix(p, axes=axes) for _, p in poses_pred.items()])
        traj_rpy(axarr_rpy, pred_rpy, '-', 'b', title='RPY', label='Ours', alpha=1.0)
        if poses_ref:
            ref_rpy = np.array([euler_from_matrix(p, axes=axes) for _, p in poses_ref.items()])
    ...
    def eval(self, toCameraCoord):
        ...
        '''
        eval_dir = self.pre_result_dir
        if not os.path.exists(eval_dir):
            os.makedirs(eval_dir)

        visual_save_root = self.visual_save_root
        if not os.path.exists(visual_save_root):
            os.makedirs(visual_save_root)
        ...
        for seq in self.eval_seqs:
            ...
            pred_file_name = self.pre_result_dir + f'/{seq}_pred.npy'
            gt_file_name = self.gt_dir + f'/{seq}.npy'
            assert os.path.exists(pred_file_name), f"File path error: {pred_file_name}"
            poses_result = self.loadPoses(pred_file_name, toCameraCoord=toCameraCoord)
            os.system('cp %s %s' % (pred_file_name, eva_seq_dir))
            if seq not in self.seqs_with_gt:
                self.calcSequenceErrors(poses_result, poses_result)
                print(f"\nSequence: {seq}")
                print(f'Distance (m): {self.distance:d}')
                print(f'Max speed (km/h): {self.max_speed * 3.6:d}')
            ...
            save_txt = os.path.join(self.pre_result_dir, 'output.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gt_dir', type=str, default='data/odometry_gt',
